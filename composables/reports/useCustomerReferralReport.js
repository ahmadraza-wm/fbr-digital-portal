...
export const useCustomerReferralReport = () => {
  const {
    loading,
    showLoading,
    hideLoading,
  ...
  const userList = ref([]);
    ...
    const fetchCustomerReferralReport = async (filters) => {
    try {
      loading.value = true;

        const response = await $axios.get("/customer/referral/report", {
        params: filters,
        });
    ...
    const exportCustomerReferralReport = async (filters = {}) => {
