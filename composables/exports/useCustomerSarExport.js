...
export const useCustomerSarExport = () => {
      const {
        loading,
        showLoading,
        hideLoading,
    ...
    const exportCustomerSar = async(filters = {}) => {
        // if (!filters.category) {
        //     showError("Please select a category before exporting.");
        //     return; 
        // }
            showLoading("Exporting customer external sar report");
            try {
                const response = await $axios.post(`/customer/external/sar/export`, filters, {
                    responseType: "text", 
                    headers: { Accept: "application/json, text/csv" }, 
                });
                const contentType = response.headers["content-type"] || "text/csv";
                let fileName = "customer_sar_export.csv";
                ...
                showSuccess("Customer external sar report exported successfully.");
            } 
            catch (error) {
