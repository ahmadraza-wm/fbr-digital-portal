...
export const useCustomerExpiryDocument = () => {
      const {
        loading,
        showLoading,
        hideLoading,
    ...
    const exportCustomerExpiryDocs = async(filters = {}) => {
            showLoading("Exporting customer documents near expiry..");
            try {
                const response = await $axios.post(`/customer/documents/near/expiry/export`, filters, {
                    responseType: "text", 
                    headers: { Accept: "application/json, text/csv" }, 
                });
                const contentType = response.headers["content-type"] || "text/csv";
                let fileName = "customer_documents_expiry_export.csv";
                ...
                showSuccess("Customer documents near expiry exported successfully.");
            } 
            catch (error) {
