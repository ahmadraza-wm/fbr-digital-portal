...
const { assignPermissionsByUserId } = useCompanies();
...
const unassign_per = computed(() => props.user_permissions || [])
...
const handleAssign = async() => {
  ...
  await assignPermissionsByUserId(userId.value, selectedPermissions.value);
...
<template>
  <VCard>
    <v-card-title class="border-b-sm bg-secondary">
      <h4 class="text-h5 py-3 text-white">Unassign Permissions</h4>
    </v-card-title>
    <VCardText class="px-4 pt-4 pb-4">
      <VRow>
        ...
        <VCol cols="12" class="d-flex justify-end">
          <VBtn variant="elevated" color="primary" @click="verifying = true" :loading="loading" v-if="permissionStore.hasPermission('user_permissions.assign')">
            Assign Permission(s)
          </VBtn>
        </VCol>
      </VRow>
