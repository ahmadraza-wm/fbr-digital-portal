...
const unassign_per = computed(() => agentPermissions.value.unassigned_permissions || [])

console.log('unassign permissions',agentPermissions.value.unassigned_permissions );
...
const assignPermissions = async () => {
  ...
  try {
    const response = await $axios.patch(`/agent/permissions/assign/${props.userId}`, {
  ...
  <VCard class="mt-5 mb-5">
    ...
    <VCardText class="px-4 pt-4 pb-4">
      <VRow>
        ...
        <VCol cols="12" class="d-flex justify-end" v-if="permissionStore.hasPermission('agent.permissions.assign')">
