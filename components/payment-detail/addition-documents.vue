...
const additionalDocument = ref(false);
...
const emit = defineEmits(["refetchadditionalDocuments"]);
...
watch(
  ...
  async (newId) => {
    if (newId) {
      ...
      customerDocumentsList.value = await fetchCustomerDocuments(newId);
      const AdditionalDocuments = await fetchCustomerDocuments(newId);
      customerDocumentsList.value = AdditionalDocuments.filter(doc => doc.category === 'Supporting');
    }
  },
  { immediate: true }
);
...
async function refetchAdditionalDocuments() {
  console.log("Refetching with ID:", props.customer_id);
  const AdditionalDocuments = await fetchCustomerDocuments(props.customer_id);
  customerDocumentsList.value = AdditionalDocuments.filter(doc => doc.category === 'Supporting');
}
...
const handleSubmit = async () => {
  ...
  console.log('addition documents payload', payload);
  ...
  if (success) {
    emit("refetchadditionalDocuments");
    formRef.value.reset();
    additionalDocument.value = false;
  }
};



console.log('customer id passing for emit', props.customer_id)
...
<template>
  <VCard class="my-5 border">
    <v-card-title :class="['border-b-sm', 'd-flex', 'align-center', 'justify-space-between', colorClass, 'bg-grey']">
      <h4 class="text-h5 py-3">Additional Documents</h4>
      <VBtn color="primary" variant="elevated" prepend-icon="tabler-plus" @click="additionalDocument = true" v-if="permissionStore.hasPermission('customer.document.upload')">Add
        Documents</VBtn>
    </v-card-title>
...
<span v-else>-</span> -->
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <VBtn icon variant="text" color="medium-emphasis" @click="deleteDocument(doc.id)" class="d-none">
  ...
  <div class="text-center">
    <!-- Dialog -->
    <v-dialog v-model="additionalDocument" max-width="1000" transition="scale-transition">
      <v-card>
        <v-row>
          <v-col cols="12">
            <v-card-title class="d-flex align-center justify-space-between" style="background-color: lightgray;">
              <h4 class="text-h4 py-3">Additional Documents</h4>
              <IconBtn @click="additionalDocument = false">
                <VIcon icon="tabler-x" />
              </IconBtn>
            </v-card-title>
          </v-col>
          ...
          <VForm @submit.prevent="handleSubmit" ref="formRef">
            <v-row class="ps-0 mt-4">
              <v-col cols="12" sm="6" md="6">
                ...
                <AppAutocomplete name="documentType" v-model="documentType" :items="additionaldocumentTypes"
              ...
              <v-col cols="12" sm="6" md="6">
                ...
                <AppDialogDate name="issuanceDate" v-model="issuanceDate" :rules="[requiredValidator]"
                  placeholder="Issuance Date" />
              </v-col>
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium" v-html="requiredLabel('Expiry Date')"></span>
                <AppDialogDate name="expiryDate" v-model="expiryDate" :rules="[requiredValidator]"
                  placeholder="Expiry Date" />
              </v-col>
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium" v-html="requiredLabel('Front Side')"></span>
                <VFileInput chips multiple :rules="[requiredValidator]" v-model="frontFiles"
                  label="Choose front files" />
              </v-col>
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium">Back Side</span>
              ...
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium">Issuer Country</span>
              ...
              <VCol cols="12" md="12" class="d-flex gap-2 align-end justify-end">
                <VBtn type="reset" color="secondary" variant="tonal" @click="additionalDocument = false">
                  Discard
                </VBtn>
    ...
    <v-dialog v-model="frontView" max-width="1000" transition="scale-transition">
      <v-card>
        <v-row>
          <v-col cols="12">
            <v-card-title class="d-flex align-center justify-space-between" style="background-color: lightgray;">
              <h4 class="text-h4 py-3">Front View
              </h4>
    ...
    <v-dialog v-model="backView" max-width="1000" transition="scale-transition">
      <v-card>
        <v-row>
          <v-col cols="12">
            <v-card-title class="d-flex align-center justify-space-between" style="background-color: lightgray;">
              <h4 class="text-h4 py-3">Back View
              </h4>
        ...
        <V-card-text>
          <VForm @submit.prevent="handleSubmit" ref="formRef">
            <v-row class="ps-0 mt-4">
              ...
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium">Document No</span>
                <AppTextField name="documentNumber" v-model="documentNumber" placeholder="1234567" />
              </v-col>
              <v-col cols="12" sm="6" md="6">
                <span class="fs-15 label-color font-weight-medium" v-html="requiredLabel('Issuance Date')"></span>
  ...
  <v-dialog v-model="isEditModalOpen" max-width="900">
    <v-card>
      ...
      <v-card-text class="pt-4">
        ...
        <EditDocumentForm :documentIdProp="selectedDocument" :documentTypesProp="additionaldocumentTypes"
          @closeModal="isEditModalOpen = false" @refetchAdditionalDocuments="refetchAdditionalDocuments" />
      </v-card-text>
    </v-card>
  </v-dialog>
  <!--Verify Document pop up design by usman faiz -->
