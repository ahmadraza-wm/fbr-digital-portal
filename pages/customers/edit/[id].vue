...
definePageMeta({ name: 'customers-edit' })

const route = useRoute()
const { getCustomer, updateCustomer } = useEnterpriseApi()
const loading = ref(false)
const fetchLoading = ref(true)
const snackbar = reactive({ show: false, text: '', color: 'success' })

const form = reactive({
  business_name: '', ntn_cnic: '', registration_type: 'Registered',
  province: '', address: '', phone: '', email: '', contact_person: '', is_active: true,
})
...
onMounted(async () => {
  ...
  catch (e) { showSnack('Failed to load customer', 'error') }
    ...
    <VRow class="mb-4">
      <VCol cols="12">
        <div class="d-flex align-center gap-3">
          ...
          <div>
            <h4 class="text-h5 font-weight-bold">Edit Customer</h4>
            <p class="text-body-2 text-medium-emphasis mb-0">Update customer information</p>
          </div>
        </div>
      </VCol>
    </VRow>
        ...
        <VCard>
          ...
          <VCardText>
            <VRow>
              ...
              <VCol cols="12" sm="6">
                <VTextField v-model="form.ntn_cnic" label="NTN / CNIC" variant="outlined" density="compact" hint="7 digits NTN or 13 digits CNIC" persistent-hint />
              </VCol>
              <VCol cols="12" sm="6">
                <VSelect v-model="form.registration_type" :items="regTypes" label="Registration Type *" variant="outlined" density="compact" />
              </VCol>
              <VCol cols="12" sm="6">
