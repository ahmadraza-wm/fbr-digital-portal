...
definePageMeta({
  middleware: "auth",
  permission: "cancel.file.for.ams.export"
})
    ...
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm">
          <h4 class="text-h5 py-3">Cancel File for Ams Export</h4>
        </v-card-title>
      </v-col>
    </v-row>
    ...
    <v-card-text class="pt-1 my-4 pb-3">
      <VForm @submit.prevent="" :key="formKey">
        <v-row class="ps-0">
          ...
          <AppTextField name="id" type="text" v-model="form.invoice_no" placeholder="**********" autocomplete="off" />
        </v-col>
          <v-col cols="12" sm="6" md="3">
          ...
          <v-col cols="12 d-flex gap-2 pt-sm-8 justify-start" sm="6" md="3">
            <VBtn type="submit" class="login-btn" variant="tonal" @click="exportCancelFileAms(form)">
              Export
            </VBtn>
          </v-col>
        </v-row>
