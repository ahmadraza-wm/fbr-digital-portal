...
definePageMeta({
  middleware: "auth",
  permission: "paid.file.for.ams.export"
})
    ...
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm">
          <h4 class="text-h5 py-3">Paid File for Ams Export</h4>
        </v-card-title>
      </v-col>
    </v-row>
      ...
      <VForm @submit.prevent="" :key="formKey">
        <v-row class="ps-0">
          ...
          <v-col cols="12 d-flex gap-2 pt-sm-8 justify-start" sm="6" md="3">
            <VBtn type="submit" class="login-btn" variant="tonal" @click="exportPaidFileAms(form)">
              Export
            </VBtn>
          </v-col>
        </v-row>
