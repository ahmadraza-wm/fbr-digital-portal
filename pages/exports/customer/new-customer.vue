...
definePageMeta({
  middleware: "auth",
  permission: "new.customer.export"
})
import { useNewCustomerExport } from '@/composables/exports/useNewCustomerExport';
const {
  exportNewCustomer
} = useNewCustomerExport();
    ...
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm">
          <h4 class="text-h5 py-3">New Customer Export</h4>
        </v-card-title>
      </v-col>
    </v-row>
      ...
      <VForm @submit.prevent="" :key="formKey">
        <v-row class="ps-0">
          ...
          <v-col cols="12 d-flex gap-2 pt-md-8 justify-start" sm="6" md="2">
            <VBtn type="submit" class="login-btn" variant="tonal" @click="exportNewCustomer(form)">
              Export
            </VBtn>
          </v-col>
        </v-row>
