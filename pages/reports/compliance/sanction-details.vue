...
definePageMeta({
  middleware: "auth",
  permission: "sanction.detail.report"
})
...
{
  title: "Sender Sanction Remarks",
  key: "sender_sanction_remarks",
},
{
  title: "Receiver",
  key: "receiver",
...
{
  title: "Receiver Sanction Remarks",
  key: "receiver_sanction_remarks",
},
...
<template>
  <v-card>
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm">
          <h4 class="text-h5 py-3">Sanction Details Report</h4>
        </v-card-title>
      </v-col>
    </v-row>
    <v-card-text>
      <VForm @submit.prevent="" :key="formKey">
        <v-row>
          ...
          <v-col cols="12" md="4" class="d-flex gap-4 pt-md-8">
            <VBtn type="submit" @click="fetchSanctionDetail(form)" variant="tonal" class="login-btn">Search</VBtn>
    ...
    <VCardText class="flex-wrap gap-4 d-flex justify-space-between align-center border-t-sm">
      ...
      <div class="gap-3 pt-6" v-if="permissionStore.hasPermission('sanction.detail.report.export')">
        <VBtn variant="elevated" color="secondary" v-if="userList.length" @click="exportSanctionDetailReport(form)">
