...
const deletePartner = async (partnerId) => {
  await deleteBankPartner(route.params.id, partnerId);
};
const options = ref({
  page: 1,
  itemsPerPage: 5,
...
const handleSubmit = async () => {
  ...
  const isSaved = await saveBankPartner(isEdit.value, form.value, route.params.id, editingItemId.value);
  if (isSaved) {
    resetForm();
  }
};
...
<template>
  <v-card>
    ...
    <VCardText class="px-0">
      <PaginatedDataTable :headers="headers" :items="BanksList" :options="options" :loading="loading" :search="search"
        ...
        <template #item.action="{ item }">
          <div class="d-flex gap-1">
            <IconBtn @click="editPartner(item)" v-if="permissionStore.hasPermission('bank_partners.update')">
              <VIcon icon="tabler-edit" color="primary" />
            </IconBtn>

            <IconBtn @click="deletePartner(item.id)" v-if="permissionStore.hasPermission('bank_partners.delete')">
          ...
          <VCardText class="pt-2 pb-0 border-t-sm">
            <div class="d-flex flex-wrap justify-space-between align-center gap-y-2 mt-2">
              <span class="text-muted">
                ...
                to
                {{ Math.min(options.page * options.itemsPerPage, BanksList.length) }}
                of {{ BanksList.length }} entries
              </span>
              <VPagination v-model="options.page" :total-visible="$vuetify.display.smAndDown ? 3 : 5"
                :length="Math.ceil(BanksList.length / options.itemsPerPage)" />
            </div>
          </VCardText>
        </template>
      </PaginatedDataTable>
