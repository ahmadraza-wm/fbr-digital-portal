    ...
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm" style="background-color: lightgrey;">
          <h4 class="text-h5 py-3">Customers List</h4>
        </v-card-title>
      </v-col>
    </v-row>
    ...
    <VDatableComponent :headers="headers" :items="customersList" :loading="loading" :search="search" :pagination="pagination"
      ...
      <template #item.action="{ item }">
        <div class="d-flex gap-2">
          <VBtn color="warning" type="button" :to="``" variant="elevated" size="small">
            Select
          </VBtn>
          <VBtn color="primary" type="button" :to="`/agent/customers/edit/${item.id}`" variant="elevated" size="small">
            Edit
          </VBtn>
        </div>
      </template>
