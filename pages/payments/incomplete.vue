...
const defaultFilters = { status: ["incomplete"] };
...
const onFilterUpdated = (filters) => {
  fetchTransactions({ status: ["incomplete"], ...filters });
};


const onFilterReset = () => {
  fetchTransactions({ status: ["incomplete"] });
};
</script>
...
<template>
  ...
  <v-card class="mt-5">
    <v-row>
      <v-col cols="12">
        <v-card-title class="border-b-sm">
          <h4 class="text-h5 py-3">Incomplete Payments</h4>
        </v-card-title>
      </v-col>
    </v-row>
    <V-row>
    ...
    <VDatableComponent :headers="headers" :items="payments" :loading="loading" :search="search" :pagination="pagination"
      :pageNo="pageNo" @update:pageNo="pageNo = $event" class="border-t">
